...
directory "#{node['sbt-extras']['setup_dir']}/.lib" do
  recursive true
...
execute "Forcing sbt-extras to install its default sbt version" do
  command "#{script_absolute_path} #{node['sbt-extras']['sbt_opts']} -batch -sbt-create help"
...
if node['sbt-extras']['preinstall']
  directory tmp_dir do
    # Create a very-temporary folder to store dummy project files, created by '-sbt-create' arg
    mode '0777'
  end
  node['sbt-extras']['preinstall'].keys.each do |sbt_user|
    node['sbt-extras']['preinstall'][sbt_user].keys.each do |sbt_version|

#FIXME      if node['sbt-extras']['preinstall'][sbt_user][sbt_version].is_a?(Hash) 

        # Download and pre-install sbt/scala version matrix 
        node['sbt-extras']['preinstall'][sbt_user][sbt_version].each do |scala_version|
          execute "running sbt-extras as user #{sbt_user} to pre-install libraries for building scala #{scala_version} with sbt #{sbt_version}" do
            ...
            command "#{script_absolute_path} #{node['sbt-extras']['sbt_opts']} -batch -sbt-version #{sbt_version} -scala-version #{scala_version} -sbt-create help"
            user    sbt_user
            group   node['sbt-extras']['group']
            umask   '002'   # grant write permission to group.
            cwd     tmp_dir
            ...
            #TODO: set a timeout node['sbt-extras']['preinstall'][:timeout]
            not_if do 
